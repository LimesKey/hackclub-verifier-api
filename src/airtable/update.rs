...
pub async fn update_submission(airtable_key: &String, record_id: &str, authenticated: bool) -> Result<String, Box<dyn Error>> {
    handle_update_request(record_id, authenticated, &airtable_key).await
}

async fn handle_update_request(record_id: &str, authenticated: bool, airtable_key: &str) -> Result<String, Box<dyn Error>> {
    ...
    if response.status().is_success() {
        Ok(format!("Record updated to [{}] successfully", fields["Authenticated"].as_str().unwrap_or("")))
    } else {
